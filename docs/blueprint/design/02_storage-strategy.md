...
- **Công nghệ Lựa chọn:** **Redis** (Lưu trữ Key-Value trên RAM - NoSQL) với 3 vai trò logic:
  ...
  - **DB1 — Message Queue:** Redis Streams cho async processing (AI summary, notification dispatch). `maxmemory-policy: noeviction`.
...
1. **Redis Streams cho async processing (ADR-10)**
   - **Stream keys:** `stream:ai-summary`, `stream:notifications`, `stream:payments-expiry`.
   - **DLQ keys:** `stream:ai-summary-dlq`, `stream:notifications-dlq`.
   - **Pattern:** Consumer Group với XREADGROUP + XACK + PEL.
   - **Crash recovery:** XAUTOCLAIM sau 60s — pending message bị claim lại.
   - **Retry:** 3 lần → XADD vào DLQ → admin can thiệp thủ công.
